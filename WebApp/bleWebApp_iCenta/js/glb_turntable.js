document.addEventListener("DOMContentLoaded", async function () {
    ...
    // user-defined input box
    const speedInput = document.createElement("input");
    speedInput.type = "number";
    speedInput.placeholder = "Enter speed";
    speedInput.style.width = "100px";
    speedInput.disabled = true;   // disabled by default

    // enable/disable input when selecting "user_define"
    speedSelect.addEventListener("change", () => {
        if (speedSelect.value === "user_define") {
    ...
    sendButton.addEventListener("click", function () {
        ...
        if (speed === "user_define") {
            s = speedInput.value.trim();
            if (s === "") {
                alert("Please enter a speed value");
                return;
            }
        } else {
            s = speed;  // "fast" or "slow"
        }
